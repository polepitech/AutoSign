...
async function signerPresence() {
  ...
  const cookieHeader2 = decodeURIComponent(cookieHeader);
  const csrfToken2 = decodeURIComponent(csrfToken);
  ...
  console.log(`✅ Réponse EdSquare (${response.status}):\n`, text);
}

signerPresence().catch((err) => {
  console.error('❌ Erreur pendant la signature :', err);
});
